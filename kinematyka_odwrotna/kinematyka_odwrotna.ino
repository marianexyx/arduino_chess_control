...
cServoA ServoA_kp(2, "kp", 15, 175, &LCD_angle, &LCD_pos);
cServoB ServoB_alpha(3, "alpha", 0, 180, &LCD_angle, &LCD_pos); // !!za duże wartości!!
cServoC ServoC_beta(4, "beta", 24, 157, &LCD_angle, &LCD_pos);
cServoD ServoD_fi(5, "fi", 45, 180, &LCD_angle, &LCD_pos);
cServoE ServoE_ks1(6, "ks1", 82, 131, &LCD_angle, &LCD_pos); //nie potrzeba by działało dla szerszych wartości
cServoF ServoF_ks2(7, "ks2", 200-131, 200-82, &LCD_angle, &LCD_pos); //2gie servo w szczekach jest obrocone(180 stopno) i przesuniete o 20 stopni w dzialaniu
...
void setup() //kod kóry wykona się raz
{
  ...
  ServoD_fi.UstawKat(90 + n_fi_poprawka, MOTOR_SPEED_FAST); /* !!! */  LCD_angle.PrintAngle("ServoD_fi", ServoD_fi.getKat());
...
void loop() //wieczna główna pętla programu
{
  ...
  if (sCoreCommand != "")
  {
    ...
    switch (KomendaRdzenia)
    {
      ...
      case CC_TRASH: sCoreCommand = ServoA_kp_kp.Trash(); break; //ustaw się nad pojemnik ze zbitymi bierkiami. metoda trash ustawi na kp, a zwrot z f da y,z
      case CC_RESET:
        Reset();
